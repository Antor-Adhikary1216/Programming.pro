      ...
      <section className="bg-cream py-20 text-canvas sm:py-28">
        <Container>
          <div className="grid gap-14 lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <article data-reveal>
              <h2 className="font-display text-4xl font-semibold tracking-[-0.045em]">Built as one product—not two disconnected halves.</h2>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{project.description}</p>
              <div className="mt-10 grid gap-3 sm:grid-cols-2">
                {['Responsive product experience', 'Validated API contracts', 'Purposeful data model', 'Production-aware configuration'].map((item) => (
                  <div key={item} className="flex items-center gap-3 rounded-2xl border border-canvas/10 bg-white p-4 text-sm font-semibold">
                    <span className="grid size-8 place-items-center rounded-lg bg-lime text-canvas"><FaCheck /></span>
                    {item}
                  </div>
                ))}
              </div>
